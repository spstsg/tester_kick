...
class SearchService {
  ...
  Future<List<String>> searchClubs(String name) async {
    ...
    QuerySnapshot clubs = await firestore
        .collection('england')
        .where('lowercaseName', isGreaterThanOrEqualTo: searchTerm)
        .get();
  ...
  Future<List<Room>> searchLiveAudioRooms(String name) async {
    ...
    QuerySnapshot audios =
        await firestore.collection(AUDIO_LIVE_ROOMS).where('tags', arrayContains: searchTerm).get();
  ...
  Future<List<User>> filterByClubName(String name) async {
    ...
    QuerySnapshot result = await firestore
        .collection(USERS)
        .where('lowercaseTeam', isGreaterThanOrEqualTo: searchTerm)
        .get();
