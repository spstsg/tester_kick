...
class LiveGamesScreen extends StatefulWidget {
  const LiveGamesScreen({Key? key}) : super(key: key);
...
class _LiveGamesScreenState extends State<LiveGamesScreen> {
  ...
  List groupKeys = [];
  late Stream<dynamic> liveMatchesFuture;
  String selectedValue = 'Filter by country';

  @override
  void initState() {
    liveMatchesFuture = _liveScores.getLiveMatches();
    super.initState();
  }

  @override
  void dispose() {
    _liveScores.disposeLiveMatchesStream();
    super.dispose();
  }
    ...
    return Container(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 10),
          Expanded(child: _liveMatches()),
        ],
      ),
    );
  }

  StreamBuilder<dynamic> _liveMatches() {
    return StreamBuilder<dynamic>(
      stream: liveMatchesFuture,
      initialData: null,
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          if (selectedValue == 'All live games' || selectedValue == 'Filter by country') {
                ...
                children: [
                  ...
                  for (var key in groupedData[group])
                    CardEventItemNew(
                      ...
                      timeMatch: key['fixture']['status']['elapsed'].toString(),
  ...
  Future<bool> setCountryFilter(String country) async {
    return _sharedPreferences.setSharedPreferencesString('LiveGamesCountry', country);
  }
        ...
        child: DropdownButton(
          ...
          onChanged: (String? newValue) async {
            ...
            if (getCountryName.isEmpty || getCountryName == '') {
              ...
              setState(() {
                selectedValue = newValue == 'All live games' ? 'Filter by country' : newValue!;
